...
{
    int result;
    std::cout <<"Cколько камней ты хочешь взять? 1? 2? 3?" << std::endl;
    std::string input;
    ...
    while ( f || result < 1 || result > 3) {//обработка некоректного ввода используя строку
        std::cerr << "ОТ 1 ДО 3! Зачем ты выбрал больше? Попробуй еще раз: " << std::endl;
    	std::getline(std::cin, input);
    	std::stringstream input_ss { input };
        input_ss >> result;
        f = input_ss.fail();
...
int lvl_1(bool f)
{
	...
	for (unsigned i=0; d1>0; ++i)
	{
		if( i%2 == 0 )
	    {
	        ...
	        while (a > d1)
	        {
	        	std::cerr << "Осталось " << d1 << " камней. Сделай другой выбор!" << std::endl;
	        	a = read_answer();
	        }
	...
	if(f1) std::cout << "Игра оконечна. Ты проиграл" << std::endl;
	else std::cout << "Игра оконечна. Ты победил! Ехуу" << std::endl;
	return 1;
}
void lvl_3(bool f, int ans)
{
	...
	for (unsigned i=0; d1>0; ++i)
	{
		if(i%2 == 0)
	    {
	        ...
	        while (a > d1)
	        {
	        	std::cerr << "Всего осталось " << d1 << " камней. Сделай другой выбор!";
	        	a = read_answer();
	        }
	        std::cout << "Осталось " <<(d1 - a) <<" камней " << std::endl;
	        d1 -= a;
...
int main()
{
	...
	do
	{
		...
		while (std::cin.fail() or choise < 1 or choise > 4) // цикл продолжается до тех пор, пока пользователь не введет корректное значение
		    {
				std::cerr << "Некорректный ввод. Попробуй еще раз :" << std::endl;
